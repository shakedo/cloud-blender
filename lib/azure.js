               ...
               azureStorage.createTable('nodesIps', function (err, res) {
                  ...
                  createCLoudServices(settings, function (err, result) {
                     ...
                     underscore.forEach(result, function (cloudService) {
                        ...
                        console.log('result.indexOf(cloudService)-'+result.indexOf(cloudService))
      ...
      createNode: function (settings, cloudServicesTestSettings, nodeIndex, callback) {
         ...
         if (cloudService[0].minIndex === nodeIndex) {
            ...
            getNodePIp(settings, cloudService[0].id, cloudService[0].deployment, cloudService[0].deploymentNode.nodeName, 40, interval, function (err, resultIp) {
               if (err) {
                  console.log('err get node IP-'+err)
               ...
               azureStorage.addNodeIp(settings.regionContext.cloudRegion, cloudService[0].deploymentNode.nodeName, resultIp, function (err, resultIp) {
                  ...
                  var resultNode = {rawResult:'node-'+cloudService[0].deploymentNode.nodeName+' was created.', node: {id: cloudService[0].deploymentNode.nodeName, status: ((cloudService[0].deploymentNode.launchStatus === 'OK') ? 'Starting' : 'ERROR_' + cloudService[0].deploymentNode.launchStatus), addresses: null, tags: cloudServicesTestSettings[0].deploymentNode.tags}};
