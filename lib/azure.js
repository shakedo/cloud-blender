      ...
      azureGetRequest(getSettings, function (err, resultNodes) {
         ...
         if (resultNodes.Deployment.RoleInstanceList[0] && resultNodes.Deployment.RoleInstanceList[0].RoleInstance) {
            ...
            if (!nodeCheck) {
               ...
               PIp = nodeCheck._wrapped[0].PublicIPs[0].PublicIP[0].Address[0];
               callback(null, PIp);
               return;
            }
         }
   ...
   var that = {
         ...
         getImageOsType(settings,  settings.nodeParams.imageId , function (err, res) {
            ...
            if (cloudService[0].minIndex === nodeIndex) {
               ...
               getNodePIp(settings, cloudService[0].id, cloudService[0].deployment, cloudService[0].deploymentNode.nodeName, 40, interval, function (err, resultIp) {
                  ...
                  azureStorage.addNodeIp(settings.regionContext.cloudRegion, cloudService[0].deploymentNode.nodeName, resultIp, function (err, resultIp) {
                     if (err) {
                        callback('err add node IP-' + err);
                        return;

                     }
                     var resultNode = {rawResult: 'node-' + cloudService[0].deploymentNode.nodeName + ' was created.', node: {id: cloudService[0].deploymentNode.nodeName, status: ((cloudService[0].deploymentNode.launchStatus === 'OK') ? 'Starting' : 'ERROR_' + cloudService[0].deploymentNode.launchStatus), addresses: null, tags: cloudService[0].deploymentNode.tags}};
               ...
               azureRetryRequest(postSettings, 40, interval, function (err, result) {
                  ...
                  azureStorage.addNodeTagging(settings.regionContext.cloudRegion, nodeName, settings.nodeParams.tags, launchStatus, cloudServiceSetting, function (err, tagRetval) {
                     ...
                     getNodePIp(settings, cloudService[0].id, cloudService[0].deployment, nodeName, 40, interval, function (err, resultIp) {
                        ...
                        console.log('resultIp-'+JSON.stringify(resultIp))

                        azureStorage.addNodeIp(settings.regionContext.cloudRegion, nodeName, resultIp, function (err, resultIp) {
                           if (err) {
                              callback('err add node IP-' + err);
                              return;
      ...
      deleteNode: function (settings, callback) {
         ...
         azureStorage.getNodeTagging(settings.regionContext.cloudRegion, settings.node.id, function (err, tagRetval) {
            ...
            if (err) {
               callback(err, {rawResult: 'node-' + -settings.node.id + '  tagging is missing'});
               return;
