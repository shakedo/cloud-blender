...
module.exports = (function () {
   ...
   function azureRetryDeleteRequest(settings, pollingCount, interval, callback) {
      ...
      request(deleteRequestSettings, function (err, res, body) {
         ...
         if (!res){
            callback(new Error('response is not valid-'+err));
            return;
         }
   ...
   function azureValidateRetryRequest(settings, pollingCount, interval, callback) {
      ...
      request[settings.restType]({
         ...
         if (res) {
            ...
            if ((err || underscore.contains(settings.successCode, res.statusCode) === false) && underscore.contains(settings.retryCodes, res.statusCode) === false) {
               ...
               azureGetOpertionStatus(settings, res.headers['x-ms-request-id'], 15, operationValidateInterval, function (errStatus, resultStatus) {
                  ...
                  if ((pollingCount > 0 && underscore.contains(settings.retryCodes, res.statusCode) === true) || (pollingCount > 0 && underscore.contains(settings.successCode, res.statusCode) === true && resultStatus === 'Failed')) {
                     setTimeout(azureValidateRetryRequest, interval, settings, pollingCount - 1, interval, callback);
                  }
   ...
   function azureGetOpertionStatus(settings, requestId, pollingCount, interval, callback) {


      var getSettings = {
         url: 'https://management.core.windows.net/' + settings.subscriptionId + '/operations/' + requestId,
      ...
      azureGetRequest(getSettings, function (errGet, resultGet) {
         ...
         if (!resultGet){
            callback(new Error('response is not valid-'+errGet));
            return;
         }
         ...
         if ((pollingCount > 0 && !resultGet) || (pollingCount > 0 && resultGet.Operation.Status[0] === 'InProgress')) {
            setTimeout(azureGetOpertionStatus, interval, settings, requestId, pollingCount - 1, interval, callback);
         }
         ...
         getImageOsType(settings, vmImageName, function (err, res) {
            ...
            if (res === 'Linux') {
               ...
               xmlBody = '<Deployment xmlns="http://schemas.microsoft.com/windowsazure" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">' +
                  ...
                  '<AdminPassword>Storm-123</AdminPassword>' +
            ...
            azureValidateRetryRequest(postSettings, 15, operationValidateInterval, function (err, result) {
   ...
   function deleteService(settings, cloudService, deployment, callback) {
      ...
      getDeploymentNodeList(settings, cloudService, function (err, result) {
         ...
         if (underscore.filter(result.nodes,function (node) {
            ...
            var DelSettings = {
               url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/hostedservices/' + cloudService+'?comp=media',
               xMsVersion: xMsVersion,
               azureCertPath: settings.regionContext.azureCertPath,
               azureKeyPath: settings.regionContext.azureKeyPath,
               successCode: [200],
   ...
   function getImageOsType(settings, imageName, callback) {
      var imageInfo,
         getSettings = {
            ...
            url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/vmimages?location=' + settings.regionContext.cloudRegion,
            xMsVersion: xMsVersion,
            azureCertPath: settings.regionContext.azureCertPath,
            azureKeyPath: settings.regionContext.azureKeyPath,
            successCode: 200
         ...
         if (err || !tagRetval) {
            ...
            getCloudServicesByLocation(settings, function (err, res) {
               ...
               underscore.forEach(res, function (resCloudService) {
                  ...
                  var getSettings = {
                     url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/hostedservices/' + resCloudService + '/deploymentslots/Production',
                     xMsVersion: xMsVersion,
                     azureCertPath: settings.regionContext.azureCertPath,
                     azureKeyPath: settings.regionContext.azureKeyPath,
                     successCode: 200
                  ...
                  azureGetRequest(getSettings, function (err, resultNodes) {
                     ...
                     if (cloudServicIndex === numberOfCloudService) {
                        ...
                        callback([new Error('node doesn\'t exist')]);
                     }
                  })
   ...
   var that = {
      ...
      deleteNode: function (settings, callback) {
         ...
         if (!settings.node || !settings.node.id) {
            callback(new Error('missing node id input'),{rawResult: 'failed to delete node'});
            return;
         }
         ...
         getNodeInfo(settings, settings.node.id, function (err, infoResult) {
            ...
            var DelSettings = {
               url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/hostedservices/' + cloudService + '/deployments/' + deployment + '/roles/' + settings.node.id+'?comp=media',
               xMsVersion: xMsVersion,
               azureCertPath: settings.regionContext.azureCertPath,
               azureKeyPath: settings.regionContext.azureKeyPath,
               successCode: [202, 400],
            ...
            azureRetryDeleteRequest(DelSettings, 40, interval, function (err, delResRole) {
               ...
               if (delResRole === 400) {
                  ...
                  var DelSettings = {
                     url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/hostedservices/' + cloudService + '/deployments/' + deployment+'?comp=media',
                     xMsVersion: xMsVersion,
                     azureCertPath: settings.regionContext.azureCertPath,
                     azureKeyPath: settings.regionContext.azureKeyPath,
                     successCode: [202],
      ...
      deleteImage: function (settings, callback) {
         ...
         var DelSettings = {
            url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/vmimages/' + settings.imageParams.imageId+'?comp=media',
            xMsVersion: xMsVersion,
            azureCertPath: settings.regionContext.azureCertPath,
            azureKeyPath: settings.regionContext.azureKeyPath,
            successCode: [202],
            ...
            underscore.forEach(cloudServices, function (cloudService) {
               ...
               getDeploymentNodeList(settings, cloudServiceName, function (err, deploymentRes) {
                  ...
                  if (errFlag===false && !deploymentRes.nodes[0]) {
                     DelSettings = {
                        url: 'https://management.core.windows.net/' + settings.regionContext.subscriptionId + '/services/hostedservices/' + cloudServiceName+'?comp=media',
                        xMsVersion: xMsVersion,
                        azureCertPath: settings.regionContext.azureCertPath,
                        azureKeyPath: settings.regionContext.azureKeyPath,
                        successCode: [200],
