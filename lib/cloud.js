...
//         console.log('after retrieving image' + JSON.stringify(imageData, null, '   '));
   ...
   var that = {
      ...
      addLaunchPermissions: function(settings, userAccountId, callback){
         var cloud = cloudProviders[settings.regionContext.providerName];
         ...
         cloud.modifyLaunchPermissions(settings, userAccountId, true,  callback)
      },

      removeLaunchPermissions: function(settings, userAccountId, callback){
         var cloud = cloudProviders[settings.regionContext.providerName];
         if(!cloud || ! cloud.modifyLaunchPermissions){
            callback(new Error ('launch permissions not supported for this provider: ' + settings.regionContext.providerName ));
            return;
         }
         cloud.modifyLaunchPermissions(settings, userAccountId, false, callback)
      }
