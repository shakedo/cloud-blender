...
var underscore = require('underscore'),
   ...
   CBMultiError = require('./cb-error').CBMultiError,
   cloudProviders = {
      'hpcs': hpcs,
      'aws': aws,
      'hpcs_13_5': hpcs_13_5,
...
//         console.log('after retrieving image' + JSON.stringify(imageData, null, '   '));
   ...
   function callApiForEachImage(apiMethod, settings, callback ){
      var limits = settings.regionContext.limits,
         ...
         currentImageIdx = 0,
         completedCalls = 0,
         multiError = new CBMultiError('Image list api'),
         results = {},
         imageCount;
      ...
      function apiCallback(error, result, imageId) {
         completedCalls++;
         if (error) {
            multiError.addError(imageId, error);
         }
         else{
            results[imageId] = result.rawResult;
         }
         if (completedCalls === imageCount) {
            callback(multiError.length ? multiError : null, underscore.size(results) ? results : null);
         }
      }
   ...
   var that = {
      ...
      releaseAddresses: function (settings, callback) {
         ...
         cloud.getAddress(settings, function (err, res) {
            if(err){
               errors.push(err);
               callback(errors);
               return;
            ...
            for (i = 0; i < settings.ips.length; i++) {
               setTimeout(cloud.releaseAddress, releaseAddressesIntervalMS * i, {regionContext: settings.regionContext, ip: settings.ips[i], requestInfo:res[settings.ips[i]]}, releaseAddressesCB);
            }
         });
      },
      ...
      validateCredentials: function(settings, callback){
         ...
         if(!cloud || !cloud.validateCredentials){
            callback(new Error ('validate credentials not supported for this provider: ' + settings.regionContext.providerName ));
            return;
         }
      ...
      //configure account according to provider requirements -each provider will have different requirements
