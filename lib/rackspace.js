    ...
    createNode: function (settings, cloudServicesTestSettings, nodeIndex, callback) {
        ...
        connect(settings, function (error, success) {
            ...
            if (adminPass){
                ...
                nodeParams.adminPass = 'debugpass';
            }
            ...
            request(createserverRequest, function (error, response, bodyString) {
                ...
                function updateSecurityGroups() {
                    request(listPorts, function (error, response, bodyString) {
                        ...
                        underscore.each(ports, function (port) {
                            if (machineId === port.device_id) {
                                ...
                                request(updateSecurityGroupForPort, function (error, response, bodyString) {
                                    if ((error) || (response && (response.statusCode !== 200))) {
                ...
                console.log('created server with the following results: \n' + JSON.stringify(bodyString));
                ...
                if (securityGroups) {
                    ...
                    setTimeout(function(){updateSecurityGroups();}, 20*1000);
                }else{
                    return callback(null, finalResults);
                }
            });
