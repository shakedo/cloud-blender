...
module.exports = (function() {
   ...
   function canonicalRequest(settings, nowDate) {
      ...
      canonical += 'host:iam.' + settings.region + '.amazonaws.com\n' +
         'x-amz-date:' + nowDate + '\n\n';
   ...
   function generateAWSRequest(settings, callback) {
      ...
      requestSettings = {
         ...
         headers: {
            ...
            'Host': 'iam.' + settings.region + '.amazonaws.com',
            'x-amz-date': nowDate
         },
         body: body,
         timeout: 120000
...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
      ...
      //This will only work when using credentials that have permissions to call the GetUser Api.
      //by default an IAM user , does not have permissions and you get AccessDenied Error.
      getAccountId: function(settings, callback){
         ...
         generateIAMRequest(credentialValidationSettings, function(error, result) {
            if(!error){
               callback(null, {accountId: result.GetUserResponse.GetUserResult[0].User[0].UserId[0]});
            }
            else{
               callback(error);
            }
