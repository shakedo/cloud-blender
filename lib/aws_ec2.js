var request = require('request'),
   ...
   async = require('async'),
   CBError = require('./cb-error').CBError;
...
var ec2FatalErrorRegList = (function CreateEc2FatalErrorCodeRegEx(){
   var ec2FatalErrorCodes = [
         'AuthFailure',
      ...
      regList = {};
   ec2FatalErrorCodes.forEach(function(errCodeHeader){
      regList[errCodeHeader] = new RegExp('^' + errCodeHeader + '((\\.\\w+)|())$');
   });
   return regList;
})();
...
module.exports = (function() {
   ...
   function getApiVersion(serviceType){
      return serviceType === 'ec2' ? '2013-02-01' : '2010-05-08';
   }

   //in iam there are no sub codes, only exact matches
   function getIAMErrorCode(error){
      try{
         return error.providerDetails.ErrorResponse.Error[0].Code[0];
   ...
   function getIAMErrorMessage(error){
      try{
         return error.providerDetails.ErrorResponse.Error[0].Message[0];
   ...
   //in iam there are no sub codes, only exact matches
   function getEc2ErrorCode(error){
      try{
         return error.providerDetails.Response.Errors[0].Error[0].Code[0];
      }catch(e){
         return undefined;
      }
   };
   ...
   function checkForFatalErrors(errorResult){
      var errorList,
         result;

      try{
         errorList = errorResult.Response.Errors;
         result =  underscore.some(errorList, function(error){
            var code = error.Error[0].Code[0];
            return underscore.some(ec2FatalErrorRegList, function(regEx){
               return !!(regEx.exec(code));
               //return fatalError === code || code.indexOf(fatalError+'.') === 0; is this quicker then regEx
            })
         });
         return result;
      }
   ...
   function generateAWSRequest(settings, callback) {
      var body,
         ...
         uriStr,
         authHeader,
         requestUrlBase = 'https://'+ settings.serviceType + (settings.serviceType === 'ec2' ? '.' + settings.region : '') + '.amazonaws.com',
      ...
      request(requestSettings, function (error, response, bodyString) {
         ...
         parseBodyString(bodyString, function(errParsing, jsonObj){
            if(errParsing){
               ...
               error.message += 'bad parsing to XML of: ' + bodyString + ', parsing error is: ' + errorParsing.message;
            }
            if(!jsonObj || error || response.statusCode>= 300){
               errorRequest = new CBError('problem in request: ' +
                  JSON.stringify(requestSettings) + ', description: ' + bodyString +
                  (response ? response.statusCode : 'request failed') + 'requestError: ' + error,
                  jsonObj, checkForFatalErrors(jsonObj));
...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
      ...
      getAddress: function (settings, callback) {
         var resultObj = {},
            addressSettings = {
               credentials: settings.regionContext.identitySettings.credentials,
               region: settings.regionContext.computeSettings.region,
      ...
      modifyLaunchPermissions: function (settings, callback) {
         ...
         accounts.forEach(function (account, index) {
            ...
            index++;
            addLaunchPermissionsSettings.params['LaunchPermission.' + operation + '.' + index + '.' + scope] = account;
         });
      ...
      validateCredentials: function (settings, callback) {
         ...
         generateIAMRequest(credentialValidationSettings, function (error, result) {
            var match,
               result;
            if (!error) {
               callback(null, result.GetUserResponse.GetUserResult[0].User[0].UserId[0] === settings.accountId ? 0 : -1);
            }
            else {
               switch (getIAMErrorCode(error)) {
                  case  'AccessDenied':
                     //AccessDenied means that the securityCredentials are Ok but they just dont have permissiosn to perform this action.
                     ...
                     match = new RegExp('arn:aws:iam::(.*?):user').exec(getIAMErrorMessage(error));  //we must do a lazy match (.*?) because this string is duplicated in the message and we only want to extract the accountId
                     result = match && match[1] === settings.accountId ? 0 : -1;
      ...
      createSecurityGroups: function (settings, finalCallback) {
         if (!settings.securityGroups) {
            callback(new Error('No data for security group creation.'));
            return;
         }
            ...
            if(settings.removeOld){ //delete the old security group if it exists
               preTasks.push( function(callback){
                  removeSecurityGroup(settings, group, function(error){
                     ...
                     if(error && getEc2ErrorCode(error) !== 'InvalidGroup.NotFound'){
                        callback(error);
                        return;
                     }
                     callback();
      ...
      createKeyPairs: function (settings, finalCallback) {
         if (!settings.keyPairs) {
            callback(new Error('No data for key pair creation'));
            return;
         }
         //should this be parallel or series ?
            ...
            if(settings.removeOld){ //delete the old security group if it exists
               preTasks.push( function(callback){
                  removeKeyPair(settings, keyPair, function(error){
                     //if there is no security group by this name then we simply continue . Any other error we return
                     if(error && getEc2ErrorCode(error) !== 'InvalidKeyPair.NotFound'){
                        callback(error);
                        return;
                     }
                     callback();
