...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
                  ...
                  underscore.each(result.DescribeImagesResponse.imagesSet, function (imageSet) {
                     underscore.each(imageSet.item, function (item) {
                        ...
                        if (item.imageState[0] === 'available'){ //only if the image is available we can be sure that we have the snapshotId
                           var snapshotId = item.blockDeviceMapping[0].item[ebsINdex].ebs[0].snapshotId[0];
      ...
      modifyLaunchPermissions: function(settings, bAdd, callback){
         var addLaunchPermissionsSettings = {
            ...
            finalResult = {};
         var scope = settings.userAccountId === 'all' ? 'Group' : 'UserId'; //if accountId = all it means all accounts (public)
         var operation = bAdd ? 'Add' : 'Remove';
         addLaunchPermissionsSettings.params['LaunchPermission.1.' + operation + '.' + scope] = settings.userAccountId;
