...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
            ...
            if (!error) {
               ...
               switch (error.providerErrorCode) {
                  case  'AccessDenied':
                     ...
                     match = new RegExp('arn:aws:iam::(.*?):user').exec(error.providerErrorMessage);  //we must do a lazy match (.*?) because this string is duplicated in the message and we only want to extract the accountId
      ...
      createSecurityGroups: function (settings, finalCallback) {
         if (!settings.securityGroups) {
            finalCallback(new Error('No data for security group creation.'));
            return;
         }
      ...
      createKeyPairs: function (settings, finalCallback) {
         if (!settings.keyPairs) {
            finalCallback(new Error('No data for key pair creation'));
            return;
         }
         //should this be parallel or series ?
