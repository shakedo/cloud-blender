...
module.exports = (function() {
   ...
   function generateAWSRequest(settings, callback) {
      var body,
         ...
         uriStr,
         authHeader,
         requestUrlPrefix = 'https://'+ settings.serviceType + (settings.serviceType === 'ec2' ? '.' + settings.region : ''),
      ...
      requestSettings = {
         ...
         url: requestUrlPrefix + '.amazonaws.com' + settings.path + ((settings.method === 'POST')?'':('?' + uriStr)),
         headers: {
         ...
         generateIAMRequest(credentialValidationSettings, function(error, result) {
            if(!error){
               ...
               switch(getIAMErrorCode(error)){
                  case  'AccessDenied':
                     //now we try to extract the account Id from the error message , and if it matches to the accountId then we know that all credentials match
