...
module.exports = (function() {
   ...
   function checkCodeFailure(error, code){
      var   result;
      try{
         result =  underscore.some(error.providerDetails.Response.Errors, function(error){
            var errCode = error.Error[0].Code[0];
            return !!ec2FatalErrorRegList[code].exec(errCode);
         });
         return result
...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
      ...
      //returns true if authenticated , false if not authenticated and error for any other error.
      //we want to differentiate between authentication error and other errors for example network error
      validateCredentials: function(settings, callback){
         that.describeAZs(settings, function(error){
            if(!error){
               return callback(null, true);
            }
            else if(checkCodeFailure(error, 'AuthFailure')){
               return callback(null, false);
            }
            else {
               callback(error); //any other error means that the call failed for other reasons so we return an error.
            }
         })
      }
