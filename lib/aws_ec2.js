...
module.exports = (function() {
   ...
   function getAvailabiltyZone(region) {
      var regionAz = regionAzs[region],
         zones,
         index;
      ...
      zones = regionAzs[region].zones;
      index = regionAzs[region].counter;
      selectedRegion = zones[index%(zones.length)];
      regionAzs[region].counter ++;
...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   var that = {
      ...
      createNode: function(settings, cloudServicesTestSettings, nodeIndex, callback) {
         ...
         availabilityZone =  getAvailabiltyZone(settings.regionContext.computeSettings.region);
         if (availabilityZone) {
            generateSettings.params['Placement.AvailabilityZone'] = availabilityZone;
         }
