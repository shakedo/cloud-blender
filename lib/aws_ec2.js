...
//            console.log('tags creation failed, polling count: ' + pollingCount + ' ' +errorTags);
   ...
   function importKeyPair(settings, keyPair, callback) {
      ...
      var importKeyPair = {
         ...
         params: {
            KeyName: keyPair.name,
            PublicKeyMaterial: keyPair.publicKeyBase64
         },
            ...
            if (!error) {
               ...
               switch (getIAMErrorCode(error)) {
                  case  'AccessDenied':
                     ...
                     var accountRegx = new RegExp('arn:aws:iam::' + settings.accountId);
                     callback(null, accountRegx.exec(getIAMErrorMessage(error)) ? 0 : -1);
                     break;
